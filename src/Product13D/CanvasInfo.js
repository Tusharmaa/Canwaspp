...
import ThreeDProducts from "./ThreeDProducts";
...
function CanvasInfo({ id, image, name, priceIndia, priceNotIndia }) {
  ...
  const sendToDatabase = (e) => {
    e.preventDefault();
    if (userName === "" || review === "") {
      alert("Enter your name and comment to post your review.");
    } else {
      db.collection("Review1").add({
        userName: userName,
        review: review,
        rate: rate,
